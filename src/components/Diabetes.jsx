...
import React, { useState } from "react";
import { Activity, CheckCircle } from "lucide-react";
import { predictDiabetes as predictDiabetesAPI } from "../api";
import DiabetesImg from "../assets/diabetes.png";
...
const Diabetes = () => {
  ...
  const handleDiabetesFormChange = (key, value) => {
    setDiabetesFormData((prev) => ({ ...prev, [key]: value }));
  };

  const isDiabetesFormValid = () =>
    Object.values(diabetesFormData).every((val) => val !== "" && !isNaN(val));
  ...
  const predictDiabetes = async () => {
    ...
    try {
      ...
      const numericData = Object.fromEntries(
        Object.entries(diabetesFormData).map(([k, v]) => [k, Number(v)])
      );

      const result = await predictDiabetesAPI(numericData);
    ...
    <div className="min-h-screen bg-gradient-to-br from-teal-50 via-cyan-50 to-emerald-50">
      <div className="max-w-7xl mx-auto px-4 py-12">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-gray-600 text-lg max-w-2xl mx-auto">
            Using clinical parameters to estimate diabetes risk with AI-powered prediction.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div className="space-y-8">
            <div className="bg-white/70 backdrop-blur-sm rounded-3xl p-8 border border-white/30 shadow-2xl text-center relative overflow-hidden">
              <div className="absolute -top-10 -right-10 w-32 h-32 bg-teal-200/30 rounded-full blur-2xl"></div>
              <div className="w-full flex justify-center mb-6 relative z-10">
                <img src={DiabetesImg} alt="Diabetes" className="w-28 h-28 object-contain" />
              </div>
              ...
              <p className="text-gray-600 mb-6 relative z-10">
                Early prediction helps in prevention and timely intervention using AI.
              </p>
          ...
          <div className="bg-white rounded-3xl shadow-2xl p-8 border border-gray-100">
            ...
            <div className="space-y-6">
              {fields.map((field) => (
                <div key={field.key}>
                  ...
                  <input
                    type="number"
                    value={diabetesFormData[field.key]}
                    onChange={(e) => handleDiabetesFormChange(field.key, e.target.value)}
                    className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500"
                    placeholder={`Enter ${field.label}`}
                  />
                </div>
              ...
              <button
                onClick={predictDiabetes}
                disabled={!isDiabetesFormValid() || isLoading}
                className={`w-full py-4 mt-4 rounded-lg font-semibold text-white shadow-lg transition-transform transform hover:scale-105 ${
                  !isDiabetesFormValid() || isLoading
                    ? "bg-gray-400 cursor-not-allowed"
